...
**Date:** 2026-06-15 (signer-key-resolution authority-loop reference verifier — end-to-end validation that a key RESOLVED FROM THE PUBLISHED JWKS authenticates the envelopes the signer actually produces (the v0.2 layer; cycles-protocol#113 `getEvidenceJwks` + cycles-server#194 publish the set, design on #103 / aeoess#43). `JwksAuthorityVerifier` (test util) is a runnable spec-faithful reference for the consumer/APS side: given an envelope + a resolved JWK Set it reports exactly one of the five dispositions, reusing the PRODUCTION `CyclesEvidenceCanonicalizer` (re-derive `evidence_id` + signing bytes) and `EnvelopeSigner` (Ed25519 verify), with the normative raw-hex resolution + window-gate (`cycles_nbf_ms <= issued_at_ms AND (cycles_exp_ms absent/null OR issued_at_ms < cycles_exp_ms)`) + deterministic single-match selection. `JwksAuthorityLoopTest` runs it over the 13 golden fixtures (already byte-validated against the APS verifier) + a new `cycles-jwks.json` publishing their signer (`ec52b49b…fc43`, `x`=base64url of the raw key): all 13 resolve to `authentic`, plus the four failure modes — `binding_only` (no JWKS; valid-but-unpinned AND pinned), `signer_resolution_failed` (set unreachable/unparseable), `signer_authority_failed` (key outside window / absent from set / mismatched pin / non-integral window value), `signature_invalid` (tampered signature; tampered payload → `evidence_id` mismatch). This is the live authority loop a deployment runs (emit/sign → publish JWKS → resolve window-covering key → verify), as durable regression coverage rather than a one-time smoke; the `cycles-jwks.json` fixture doubles as the concrete key set APS can point their resolver at. codex review: integral-numeric window validation + 32-byte `x` check (no `asLong()` coercion to 0), and binding_only documented to cover valid-but-unpinned per spec. 22 tests; test-only (verifier lives in `src/test`, no production/wire/spec change, no jacoco-main impact).), 2026-06-14 (evidence identity keygen helper — `tools/EvidenceKeygen.java`, a single-file JDK source-launch helper (no build, no deps) for operators enabling CyclesEvidence on a self-hosted deployment. Generates an Ed25519 keypair and prints the three env vars (`EVIDENCE_SERVER_ID` from an argv server_id, `EVIDENCE_SIGNING_SIGNER_DID`, secret `EVIDENCE_SIGNING_PRIVATE_KEY_HEX`) in the exact 64-hex formats the signer validates — both keys the raw 32-byte tail of the DER encoding (the same extraction `LocalEvidenceSigningKey.rawTailHex` uses; empirically round-trips through `EnvelopeSigner`'s SPKI/PKCS#8 prefix path), and a sign/verify pair probe mirroring `LocalEvidenceSigningKey.pairConsistent` runs before anything prints so it can't emit a pair the worker would reject at startup. `tools/README.md` documents it + a language-agnostic OpenSSL alternative (last-32-of-DER, verified to yield the same 64-hex); the enablement runbook gains a "Generating the identity" section pointing at it. v0.1 raw-hex `signer_did` path only (the did:cycles/JWKS form is the v0.2 layer, cycles-protocol#103). Operator tooling outside `src/` — not in the Maven reactor or jacoco bundle; no service/wire/spec change.), 2026-06-14 (evidence worker integration test — extended `EvidenceWorkerIntegrationTest` from the single `reserve` round-trip to ALL FIVE artifact types (decide / reserve / commit / release / error) against real Redis (Testcontainers). A shared `roundTrip(artifactType, payloadBody)` helper LPUSHes a source record shaped exactly as cycles-server's `EvidenceEmitter` produces it, lets the LIVE scheduled worker BRPOP → build → sign → store, then reads the envelope back and asserts: `artifact_type`↔`payload`-key pairing, `server_id`/`signer_did` stamped, the `evidence_id` recomputes (`CyclesEvidenceCanonicalizer`) and is 64-hex, and the Ed25519 `signature` verifies against the configured key (`EnvelopeSigner`) — plus per-type shape (commit/release hoist `reservation_id`; error carries `endpoint`+`http_status`). Each case `flushAll`s first for isolation. This is the signer-tier end-to-end guarantee across every shape cycles-server now emits (the producer side proves byte-identical `evidence_id` for the same 5 types via cycles-server's `EvidenceIdComputerTest` against the shared golden fixtures). 1 → 5 IT cases; full suite green. Test-only — no wire/spec/behavior change.), 2026-06-12 (v0.1.25.14 — evidence worker — producer evidence_id cross-check + two review fixes on `fix/evidence-worker-crosscheck-and-findings`, on the reliable-queue tip. (1) CROSS-CHECK: `EvidenceWorker.build` now compares the producer-stamped `evidence_id` (cycles-server computes it SYNCHRONOUSLY and returns it on the reserve response — cycles-server#177) against the id this worker recomputes; a mismatch (server-id/signer-did config drift between producer and worker) dead-letters the record rather than storing an envelope the caller can't bind to. Records without `evidence_id` (producer identity unconfigured) skip the check. (2) Finding [Medium]: split store from ack in `processNext` — a post-store ack failure now leaves the record in processing for `recover()` to requeue (reprocessing is idempotent, content-addressed) instead of dead-lettering an already-stored envelope and clearing it. (3) Finding [Low]: `artifact_type` upper-cases under `Locale.ROOT`, so a special-casing default locale (Turkish dotless-i: `decide` → `DECİDE`) can't break the enum lookup and wrongly dead-letter a valid record. 4 new worker tests (cross-check match/mismatch, stored-then-ack-fails, locale-independent enum); full `mvn verify` green (253 tests, jacoco 95% gate met). No wire/spec change.), 2026-06-12 (evidence reliable queue — review fix on `feat/cycles-evidence-reliable-queue`, stacked on the store slice. Replaces the worker's destructive BRPOP with the BLMOVE reliable-queue pattern: `EvidenceQueueConsumer.claim` atomically moves a record from `evidence:pending` to an in-flight `evidence:processing` list, and the worker `ack`s (LREM) only after the envelope is durably stored or dead-lettered — so a crash between claim and store leaves the record in processing rather than silently losing an audit artifact. `EvidenceRecovery` (`@EventListener ApplicationReadyEvent`) returns orphaned in-flight records to pending on startup; reprocessing is safe (content-addressed → idempotent). New `cycles.evidence.queue.processing-key`; OPERATIONS.md updated. Tests: consumer claim/ack/recover/dead-letter (5), worker ack-on-store + ack-on-dead-letter (5), recovery (2); Redis round-trip IT green. No wire/spec change.), 2026-06-12 (evidence store — WIP on `feat/cycles-evidence-store`, stacked on the worker slice. `EvidenceStore` interface (content-addressed `put`, keyed by `evidence_id`) with `RedisEvidenceStore` as the default backend (`@ConditionalOnProperty cycles.evidence.store.backend=redis`; SET at `evidence:envelope:<id>`, `store.ttl-seconds` default 0 = no expiry / archival; an s3/gcs impl drops in via the property) and `StoringEvidenceSink` (`@Primary` `EvidenceSink`) superseding the placeholder `LoggingEvidenceSink`. The worker now persists each built envelope; cycles-server serves it by id (`GET /v1/evidence/{id}`, next slice). 3 unit tests (store SET/SETEX, sink put); `EvidenceWorkerIntegrationTest` reworked to assert the envelope is persisted to Redis and verifies (Testcontainers round-trip, green). No wire/spec change.), 2026-06-12 (evidence worker — WIP on `feat/cycles-evidence-worker`; connects the producer to the signer. `EvidenceQueueConsumer` (BRPOP `evidence:pending`; bounded `evidence:failed` dead-letter via LTRIM `failed-max-len`) and `EvidenceWorker` (`@Scheduled`, like `DispatchLoop`: pop → validate → build+sign stamping `server_id` + `signer_did` → sink; a record with a missing payload/`artifact_type`/`issued_at_ms` or a blank `server_id` is dead-lettered rather than signed into an empty envelope); `EvidenceSink` seam + placeholder `LoggingEvidenceSink`. Bumps `spring.task.scheduling.pool.size` 3→5 (two continuous BRPOP loops now run); OPERATIONS.md gains a CyclesEvidence section. 37 evidence unit tests + `EvidenceWorkerIntegrationTest` (Testcontainers Redis round-trip); full `mvn verify` green with the jacoco 95% gate met. No wire/spec change.), 2026-06-12 (evidence envelope builder — WIP on `feat/cycles-evidence-envelope-builder`. `EvidenceArtifactType` (decide/reserve/commit/release/error; the wire name doubles as the `payload` key) and `CyclesEvidenceEnvelopeBuilder` (assembles a cycles-evidence/v0.1 envelope with the body nested under `payload.<artifact_type>`, derives `evidence_id` and the Ed25519 `signature`, returns `BuiltEvidenceEnvelope`; omits `trace_id` when blank). 7 tests, including reproducing 5 fixtures' `evidence_id` from their parts. No wire/spec change.), 2026-06-12 (evidence signing-key seam — WIP on `feat/cycles-evidence-signing-key`. `EvidenceSigningKey` interface (`signerDid` + `sign`; raw key not exposed) and `LocalEvidenceSigningKey` (config `cycles.evidence.signing.private-key-hex` + `signer-did`: both set → validated by a sign/verify probe; neither → ephemeral dev key with a warn; one → fail). Sibling-in-event-tier, swappable for a KMS impl behind the seam (cycles-protocol#103). 6 tests. No wire/spec change.), 2026-06-12 (evidence-emitter foundation — WIP on `feat/cycles-evidence-emitter`. Adds `io.runcycles.events.evidence`: `CyclesEvidenceCanonicalizer` (the cycles-evidence-v0.1 recipe — RFC 8785 JCS via `io.github.erdtman:java-json-canonicalization:1.1` + sha256 for `evidence_id`; signing input is JCS with `evidence_id` populated and `signature` emptied) and `EnvelopeSigner` (Ed25519 on the JDK EdDSA provider; raw-hex keys wrapped in fixed SPKI/PKCS#8 DER prefixes; fail-closed verify). `CyclesEvidenceCanonicalizerTest` reproduces all 13 reference fixtures' `evidence_id` and verifies their signatures, matching the Python generator and APS verifier. 15 tests. No wire/spec change.), 2026-05-25 (v0.1.25.13 — Apache Tomcat CVE patch: re-introduce `<tomcat.version>10.1.55</tomcat.version>` override to fix 3 CRITICAL + 3 HIGH + 1 LOW CVEs landed in trivy DB between 2026-05-11 and 2026-05-24 against `tomcat-embed-core 10.1.54` (SB 3.5.14's managed version). CVEs: CVE-2026-43515 / -43512 / -41293 (CRITICAL), -43513 / -42498 / -41284 (HIGH), -43514 (LOW). All in `tomcat-embed-core` 10.1.0-M1..10.1.54 range; all fixed in 10.1.55. Property-override only — no code change, no spec change, no wire change. Same pin shape as the v0.1.25.10 10.1.54 override (dropped at v0.1.25.12 when SB 3.5.14 caught up). Remove once SB ships 10.1.55+ as managed.), 2026-04-26 (v0.1.25.12 — dependency hygiene: Spring Boot 3.5.13 → 3.5.14 (patch with security fixes incl. constant-time comparison for remote DevTools secret, hostname verification, `RandomValuePropertySource` SecureRandom); Jedis 5.2.0 → 6.2.0 (major; binary compatibility for `SetParams` restored in 6.1.0; our usage `JedisPool`/`Jedis`/`SetParams`/`ScanParams`/`ScanResult`/`JedisConnectionException` is unaffected, all 199 tests pass); GHA `aquasecurity/trivy-action` 0.35.0 → 0.36.0 and `dependabot/fetch-metadata` v2 → v3; **drop `<tomcat.version>10.1.54</tomcat.version>` override** since Spring Boot 3.5.14's BOM now manages 10.1.54 — same effective Tomcat, simpler pom. No code changes; `WebhookTransport` hardcoded version fallback synced to 0.1.25.12.), 2026-04-23 (v0.1.25.11 — admin-spec v0.1.25.33 alignment, dispatcher half: emit `webhook.disabled` Event on auto-disable. When `DeliveryHandler.incrementConsecutiveFailures` crosses `disable_after_failures`, the dispatcher now writes an Event directly to the shared Redis store alongside the existing `DISABLED` status flip and `cycles_subscription_auto_disabled_total` metric. `EventRepository.save` mirrors the admin-side Lua script (`event:<id>` with TTL + ZADD on `events:<tenantId>` and `events:_all` + optional SADD on `events:correlation:<cid>`); `EventType.WEBHOOK_DISABLED` and `EventCategory.WEBHOOK` enum values added (additive, no wire break). `correlation_id = webhook_auto_disable:<subscription_id>:<delivery_id>`; payload conforms to `EventDataWebhookLifecycle` with `disable_reason="consecutive_failures_exceeded_threshold"`; `actor.type=system`, `source=cycles-events`; `trace_id` copied from the triggering Delivery when present. Emit is best-effort (Redis write failure is logged at WARN but does not revert the status flip). The operator-initiated webhook lifecycle emits — `webhook.created/updated/paused/resumed/deleted` — remain the responsibility of `cycles-server-admin` v0.1.25.39; this patch closes only the auto-disable gap the spec names as the dispatcher's exclusive emission point.), 2026-04-19 (v0.1.25.10 — supply-chain CVE fix; Spring Boot 3.5.11 → 3.5.13 + `<tomcat.version>10.1.54</tomcat.version>` pin closes 4 HIGH/CRITICAL CVEs on `tomcat-embed-core`: CVE-2026-29145 CRITICAL, CVE-2026-29129 HIGH (SB 3.5.13 transitive 10.1.53), CVE-2026-34483 HIGH, CVE-2026-34487 HIGH (10.1.54 pin). No code changes; all 195 tests pass.),
