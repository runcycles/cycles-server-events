...
**Date:** 2026-06-12 (evidence worker — WIP on `feat/cycles-evidence-worker`, stacked on the envelope-builder slice; CLOSES THE PRODUCER→SIGNER LOOP. Adds `EvidenceQueueConsumer` (BRPOP `evidence:pending`, the queue cycles-server feeds — mirrors the webhook `DeliveryQueueRepository`), `EvidenceWorker` (`@Component`, `@Scheduled(fixedDelay=1)` like `DispatchLoop`: pops a source record, maps `artifact_type`/`issued_at_ms`/`trace_id`/`payload` → `CyclesEvidenceEnvelopeBuilder.build(...)` stamping `server_id` (config `cycles.evidence.server-id`) + `signer_did` (the signing key), hands the built envelope to the sink; a record that fails to build/sign is DEAD-LETTERED to `evidence:failed` (LPUSH, not silently dropped — evidence is an audit trail) and the loop continues), and the `EvidenceSink` seam with a placeholder `LoggingEvidenceSink` (the persistence/serving slices contribute the real content-addressed store behind it via `@Primary`). The sibling-in-event-tier signing worker now owns the server identity it stamps; cycles-server emits only operational facts (its producer half = cycles-server PR #175). 7 tests: worker builds + sinks a self-consistent VERIFIABLE signed envelope from a source record (evidence_id recomputes, signature verifies against signer_did), empty-queue no-op, malformed → dead-letter (verified); BRPOP value/timeout mapping + dead-letter LPUSH (mocked Jedis); sink no-throw. **Input-validation hardening (review-2):** the worker now REFUSES to sign garbage — a missing/non-object `payload`, missing `artifact_type`/`issued_at_ms`, or an unconfigured (blank) `server_id` all dead-letter instead of producing a validly-signed-but-empty envelope (empirically: a payload-less record was signing `payload:{reserve:null}`); a startup WARN fires on blank `server_id`. The dead-letter queue is BOUNDED (LTRIM to `failed-max-len`, default 10000) so a flood can't grow Redis unboundedly. Adds `EvidenceWorkerIntegrationTest` (Testcontainers Redis round-trip: LPUSH source record → scheduled worker → built+signed envelope verifies; `*IntegrationTest`, Docker/CI only, excluded from unit runs). 37 evidence unit tests + the integration test. Also bumps `spring.task.scheduling.pool.size` 3→5 (env `SCHEDULING_POOL_SIZE`) — a second continuous BRPOP loop now runs alongside `DispatchLoop`, so the pool must not starve the periodic tasks; OPERATIONS.md gains a CyclesEvidence operations section (signing-key provisioning, ephemeral-key per-replica caveat, server_id, `evidence:failed` monitoring, pool sizing). No wire change; no spec change. END-TO-END reserve→signed-envelope path now exists (durable storage + GET serving still to come).), 2026-06-12 (evidence envelope builder — WIP on `feat/cycles-evidence-envelope-builder`, stacked on the signing-key slice. Adds `EvidenceArtifactType` (decide/reserve/commit/release/error; the wire name doubles as the `payload` key) and `CyclesEvidenceEnvelopeBuilder` (`@Component`): assembles a `cycles-evidence/v0.1` envelope from lifecycle metadata + the artifact-specific payload body, nesting the body under `payload.<artifact_type>` (the NORMATIVE pairing, enforced by construction), then derives `evidence_id` (via `CyclesEvidenceCanonicalizer`) and the Ed25519 `signature` (via `EvidenceSigningKey`) and returns a finished `BuiltEvidenceEnvelope` (evidence_id + node + JSON bytes to persist/serve). Omits `trace_id` when blank. 7 tests: assembly-correctness reproduces the `evidence_id` of 5 reference fixtures byte-for-byte FROM THEIR PARTS (decide/reserve/commit/release + no-trace-id) — proving spec-exact envelope layout — plus a self-consistent signed build and trace-id omission. Combined evidence suite 28/28. No wire change; no spec change.), 2026-06-12 (evidence signing-key seam — WIP on `feat/cycles-evidence-signing-key`, stacked on the foundation slice. Adds the `EvidenceSigningKey` interface (clean seam: `signerDid()` + `sign(byte[])`; the raw private key never crosses it) and `LocalEvidenceSigningKey` (`@Component`) — a config-backed Ed25519 identity via `cycles.evidence.signing.private-key-hex` + `signer-did`: both set → validated by a sign/verify probe at startup; neither → an ephemeral dev key with a WARN; exactly one → fail-closed half-config error. Sibling-in-event-tier per the architecture decision, swappable for a KMS / dedicated-signer impl behind the seam once key rotation lands (`cycles-protocol#103`). 6 tests (ephemeral, configured-pair, mismatch, both half-config directions, malformed hex). No wire change; no spec change.), 2026-06-12 (evidence-emitter foundation — WIP on `feat/cycles-evidence-emitter`, first slice of CyclesEvidence envelope emission: the long-pole gating `aeoess/agent-passport-system#43` (b), `cycles-protocol#103`, and the `replay_class` decision_replay→full_replay promotion. Adds package `io.runcycles.events.evidence`: `CyclesEvidenceCanonicalizer` (RFC 8785 JCS via `io.github.erdtman:java-json-canonicalization:1.1` + sha256 → `evidence_id`; signing-input bytes = JCS with `evidence_id` populated and `signature` emptied — the `cycles-evidence-v0.1` normative recipe, empty-string sentinel) and `EnvelopeSigner` (Ed25519 sign/verify on the JDK-native EdDSA provider, no third-party crypto; raw 32-byte hex keys wrapped in fixed Ed25519 SPKI/PKCS#8 DER prefixes; fail-closed `verify` on malformed key/sig). **Cross-impl byte-equality PROVEN**: `CyclesEvidenceCanonicalizerTest` reproduces all 13 `cycles-evidence-v0.1` reference fixtures' `evidence_id` byte-for-byte AND verifies their embedded signatures against `signer_did` — the Java stack now agrees with the Python generator (`jcs` + PyNaCl) and the APS JS verifier. 15/15 tests pass. FOUNDATION ONLY: not wired into the dispatch path, no envelope storage/serving, no signing-key provisioning — those land in subsequent slices. No wire change; no spec change (`cycles-evidence-v0.1` is schema-only and already authoritative). Version/inventory/coverage counts below are finalized at feature completion.), 2026-05-25 (v0.1.25.13 — Apache Tomcat CVE patch: re-introduce `<tomcat.version>10.1.55</tomcat.version>` override to fix 3 CRITICAL + 3 HIGH + 1 LOW CVEs landed in trivy DB between 2026-05-11 and 2026-05-24 against `tomcat-embed-core 10.1.54` (SB 3.5.14's managed version). CVEs: CVE-2026-43515 / -43512 / -41293 (CRITICAL), -43513 / -42498 / -41284 (HIGH), -43514 (LOW). All in `tomcat-embed-core` 10.1.0-M1..10.1.54 range; all fixed in 10.1.55. Property-override only — no code change, no spec change, no wire change. Same pin shape as the v0.1.25.10 10.1.54 override (dropped at v0.1.25.12 when SB 3.5.14 caught up). Remove once SB ships 10.1.55+ as managed.), 2026-04-26 (v0.1.25.12 — dependency hygiene: Spring Boot 3.5.13 → 3.5.14 (patch with security fixes incl. constant-time comparison for remote DevTools secret, hostname verification, `RandomValuePropertySource` SecureRandom); Jedis 5.2.0 → 6.2.0 (major; binary compatibility for `SetParams` restored in 6.1.0; our usage `JedisPool`/`Jedis`/`SetParams`/`ScanParams`/`ScanResult`/`JedisConnectionException` is unaffected, all 199 tests pass); GHA `aquasecurity/trivy-action` 0.35.0 → 0.36.0 and `dependabot/fetch-metadata` v2 → v3; **drop `<tomcat.version>10.1.54</tomcat.version>` override** since Spring Boot 3.5.14's BOM now manages 10.1.54 — same effective Tomcat, simpler pom. No code changes; `WebhookTransport` hardcoded version fallback synced to 0.1.25.12.), 2026-04-23 (v0.1.25.11 — admin-spec v0.1.25.33 alignment, dispatcher half: emit `webhook.disabled` Event on auto-disable. When `DeliveryHandler.incrementConsecutiveFailures` crosses `disable_after_failures`, the dispatcher now writes an Event directly to the shared Redis store alongside the existing `DISABLED` status flip and `cycles_subscription_auto_disabled_total` metric. `EventRepository.save` mirrors the admin-side Lua script (`event:<id>` with TTL + ZADD on `events:<tenantId>` and `events:_all` + optional SADD on `events:correlation:<cid>`); `EventType.WEBHOOK_DISABLED` and `EventCategory.WEBHOOK` enum values added (additive, no wire break). `correlation_id = webhook_auto_disable:<subscription_id>:<delivery_id>`; payload conforms to `EventDataWebhookLifecycle` with `disable_reason="consecutive_failures_exceeded_threshold"`; `actor.type=system`, `source=cycles-events`; `trace_id` copied from the triggering Delivery when present. Emit is best-effort (Redis write failure is logged at WARN but does not revert the status flip). The operator-initiated webhook lifecycle emits — `webhook.created/updated/paused/resumed/deleted` — remain the responsibility of `cycles-server-admin` v0.1.25.39; this patch closes only the auto-disable gap the spec names as the dispatcher's exclusive emission point.), 2026-04-19 (v0.1.25.10 — supply-chain CVE fix; Spring Boot 3.5.11 → 3.5.13 + `<tomcat.version>10.1.54</tomcat.version>` pin closes 4 HIGH/CRITICAL CVEs on `tomcat-embed-core`: CVE-2026-29145 CRITICAL, CVE-2026-29129 HIGH (SB 3.5.13 transitive 10.1.53), CVE-2026-34483 HIGH, CVE-2026-34487 HIGH (10.1.54 pin). No code changes; all 195 tests pass.),
